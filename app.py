...
# ---------------------------------------------------------------------------
STAFF = [
    {"name": "Clare", "full_name": "Clare Paulite", "role": "Store Manager", "category": "sales", "employment": "Permanent", "target": 24, "daily_rate": 550},
    {"name": "Jem", "full_name": "Jeremy Padayao", "role": "Sales Staff", "category": "sales", "employment": "Permanent", "target": 21, "daily_rate": 460},
    {"name": "Von", "full_name": "Yvony Nantes", "role": "Sales Staff", "category": "sales", "employment": "Permanent", "target": 21, "daily_rate": 460},
    {"name": "Jha", "full_name": "Jhannelha Opena", "role": "Sales Staff", "category": "sales_pt", "employment": "Part-time", "target": None, "daily_rate": 460},
    {"name": "Macky", "full_name": "Mark Jay Siruma", "role": "Machine Operator", "category": "machine", "employment": "Permanent", "target": None, "daily_rate": 400},
    {"name": "Joshua", "full_name": "Joshua Hermida", "role": "Machine Operator", "category": "machine", "employment": "Permanent", "target": None, "daily_rate": 350},
]
...
def generate_schedule(year, month):
    ...
    # --- Pass B: fill Clare/Jem/Von, biasing toward 2-day-off blocks,
    ...
    def day_requirement(d):
        """Shift labels Clare/Jem/Von must fill that day."""
        wd = d.weekday()
        if wd == 6:  # Sunday: 2 people total on Opening
            return [OPEN] if d not in jha_off_sundays else [OPEN, OPEN]
        if wd == 2:  # Wednesday: Jha already covers Opening
            return [CLOSE]
        return [OPEN, CLOSE]

    names3 = ["Clare", "Jem", "Von"]
...
def compute_payroll(db, start, end, pay_date):
    ...
    cur = db.cursor()
    cur.execute(
        """SELECT id, name, full_name, role, category, daily_rate,
    ...
    for s in staff_rows:
        ...
        total_deductions = sss + pagibig + philhealth + hmo + error_deduction
...
@app.route("/")
def index():
    return render_template("index.html", staff=STAFF, business_name=BUSINESS_NAME)


STAFF_EDITABLE_FIELDS = {
    "full_name", "role", "employment", "target", "daily_rate",
    "address", "phone", "email", "birthday",
    "default_sss", "default_pagibig", "default_philhealth", "default_hmo",
    "sss_id", "pagibig_id", "philhealth_id", "hmo_id",
    "bank_name", "bank_account_name", "bank_account_number",
}
STAFF_NUMERIC_FIELDS = {"daily_rate", "default_sss", "default_pagibig", "default_philhealth", "default_hmo"}
...
def api_staff_list():
    ...
    cur = db.cursor()
    cur.execute(
        """SELECT name, full_name, role, category, employment, target, daily_rate,
